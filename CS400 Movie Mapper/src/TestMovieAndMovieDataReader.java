...
public class TestMovieAndMovieDataReader {
  ...
  public void runTests() {
    ...
    if (this.testDirector()) {
      System.out.println("Test movie director: PASSED");
    } else {
      System.out.println("Test movie director: FAILED");
    }
    if (this.testAvgVote()) {
      System.out.println("Test average vote: PASSED");
    } else {
      System.out.println("Test average vote: FAILED");
    }
  }
    ...
    try {
      movieList = readerToTest.readDataSet(new StringReader(
          "title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
              ...
              + "The Insurrection,The Insurrection,2020,Action,90,USA,English,Rene Perez,Rene Perez,,\"Michale Pare, Wilma Elles, Joseph Camilleri, Rebecca Tarabocchia, Jeanine Harrington, Malorie Glavan, Danner Boyd, Michael Cendejas, Woody Clendenen, Keely Dervin, Aaron Harvey, Tony Jackson, Michael Jarrod, Angelina Karo, Bernie Kelly\",The director of the largest media company wants to expose how left-wing powers use film to control populations.,2.9\n"
  ...
  public boolean testDirector() {
    ...
    boolean check1 = (director1.equals(movieList.get(0).getDirector())
        || director1.equals(movieList.get(1).getDirector())
        || director1.equals(movieList.get(2).getDirector()));
    boolean check2 = (director2.equals(movieList.get(0).getDirector())
        || director2.equals(movieList.get(1).getDirector())
        || director2.equals(movieList.get(2).getDirector()));
    boolean check3 = (director3.equals(movieList.get(0).getDirector())
  ...
  /**
   * This test reads in 3 movies and tests whether the list of movies contains all 3 ratings of the
   * source data in any order. It fails if the list returned is missing one or more ratings or if an
   * exception occurs while reading in the data.
   * 
   * @return true if the test passed, false if it failed
   */
  public boolean testAvgVote() {
    List<MovieInterface> movieList;
    try {
      movieList = readerToTest.readDataSet(new StringReader(
          "title,original_title,year,genre,duration,country,language,director,writer,production_company,actors,description,avg_vote\n"
    ...
    boolean check1 = (rating1 == movieList.get(0).getAvgVote()
        || rating1 == movieList.get(1).getAvgVote() || rating1 == movieList.get(2).getAvgVote());
    boolean check2 = (rating2 == movieList.get(0).getAvgVote()
        || rating2 == movieList.get(1).getAvgVote() || rating2 == movieList.get(2).getAvgVote());
    boolean check3 = (rating3 == movieList.get(0).getAvgVote()
        || rating3 == movieList.get(1).getAvgVote() || rating3 == movieList.get(2).getAvgVote());
